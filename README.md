...
**2.2.2** (2026-06-20)
- refactor: Триггеры `API error` и `Low balance` переведены на чистую идиому Zabbix 7 — убрана тавтология `(x="" or x<>"")`, которая искусственно тащила лишний айтем ради позиционного макроса. `event_name` теперь использует выражение-макрос `{?last(//rr-api-error-text)}` / `{?last(//rr-currency)}` вместо `{ITEM.VALUE5}` / `{ITEM.VALUE2}`. Проверено на живом инстансе Zabbix 7.0: текст ошибки и валюта подставляются в имя проблемы и замораживаются на момент события (recovery-сообщение сохраняет проблемное значение).

**2.2.1** (2026-06-20)
- fix: услуги без срока (srv_cloudvps и др.) показывают «бессрочно» вместо сырого `0000-00-00` — в `{#EXPIRE_DATE}` (имена/event_name триггеров) и в айтеме Expiration Date
